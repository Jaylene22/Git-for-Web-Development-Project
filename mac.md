...
Git is an Open Source Distributed Version Control System, it can be used to store content. The difference between git and GitHub is GitHub helps manage Git repositories and overall makes it easier to use git. 
2. Why do we create a branch? 
A branch is an idependent line of development. It can also isolate any changes that are being made.
3. What is the purpose of a pull request? 
Pull requests lets other's know when changes have been made to a branch.
4. What is the command you can use to switch between branches? For example you are working on a feature branch and you want to switch back to main. 
A command we can use to switch between branches would either be git switch or git checkout. 
5. Explain the difference between `git fetch`, `git merge` and `git pull` what does each command do? 
Git fetch is downloads objects, git merge takes independent lines and integrate them into a single branch and  git pull is a command used to update the local version of a repository from a remote. 
6. What is a merge conflict? How do you resolve a merge conflict? 
A merge conflict is when there were changes made to the same line of a file or when someone edits a file that has been deleted. To resolve and work through these conflicts bwe can use tools such as the mergetool "git mergetool", look at diffs "git diff", look at diffs from each branch "git log --merge -p <path>", or look at the originals "git show : 1:filename". 